...
func GenerateRecoveryLink(baseURL, email string, recoveryToken string) string {
	recoveryLink := fmt.Sprintf("http://%s/recover-password?token=%s&user=%s", baseURL, recoveryToken, email)
	return recoveryLink
}
