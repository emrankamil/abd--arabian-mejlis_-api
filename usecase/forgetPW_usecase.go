...
func (fpu *forgetPWUsecase) ForgetPW(c context.Context, email string, server_address string) error {
	...
	pw_recovery_link := utils.GenerateRecoveryLink(server_address, user.Email, pw_recovery_code)
...
func (fpu *forgetPWUsecase) VerifyForgetPWRequest(c context.Context, email string, recovery_token string) error {
	ctx, cancel := context.WithTimeout(c, fpu.contextTimeout)
	defer cancel()
	user, err := fpu.userRepository.GetUserByEmail(ctx, email)
	if err != nil {
...
func (fpu *forgetPWUsecase) GenerateRecoveryLink(server_address string, email string, recoveryToken string) (recoveryLink string) {
	return utils.GenerateRecoveryLink(server_address, email, recoveryToken)
}